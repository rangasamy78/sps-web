<script type="text/javascript">
    $(function() {
        ...
        var table_sample_order_event = $('#quoteCrmEventDatatable').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('quote.events.list', ':id') }}".replace(':id', $('#quote_id').val()),
                type: 'GET',
                data: function(d) {
        ...
        $(document).on("change", ".event_check", function() {
            ...
            $.ajax({
                url: '{{ route("quote.events.update", ":id") }}'.replace(':id', id),
                type: 'PUT',
                data: data,
                dataType: 'json',
                success: function(response) {
        ...
        var table_product_list = $('#productListTable').DataTable({
            ...
            rowCallback: function(row, data, index) {
                ...
                $(row).on('click', function() {
                    if ($('#addCrmEventForm').is(':visible')) {
                        $('#addCrmEventForm #product_id').val(data.id);
                        $('#addCrmEventForm  #product_name').val(data.product_name);
