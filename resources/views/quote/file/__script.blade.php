<script type="text/javascript">
    $(function() {
        ...
        $id = $('#quote_id').val();
        var table_quote_file = $('#quoteFileDatatable').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('quote.quote_file.list', ':id') }}".replace(':id', $id),
                type: 'GET',
                data: function(d) {
        ...
        $('#saveFile').click(function(e) {
            e.preventDefault();
            var form = $('#uploadQuoteFileForm')[0];
            var formData = new FormData(form);
            $.ajax({
                url: '{{ route("quote.files.store") }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        table_quote_file.draw();
                        $('#uploadQuoteFileForm')[0].reset();
                    } else {
        ...
        $(document).on('click', '.updateFile', function(e) {
            ...
            $.ajax({
                url: "{{ route('quote.files.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response.status === 'success') {
        ...
        function deletevisitFile(id) {
            var url = "{{ route('quote.files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
                ...
                success: function(response) {
                    if (response.status === "success") {
                        handleAjaxResponse(response, table_quote_file);
                    } else {
