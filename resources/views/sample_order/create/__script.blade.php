<script type="text/javascript">
    $(function() {
        ...
        $('#formAddNewSampleOrder input, #formAddNewSampleOrder select').on('keyup change', function() {
        ...
        $('#nextStepAddProductBtn').click(function(e) {
            ...
            var button = $(this);
            sending(button);
            var url = $('#sample_order_id').val() ? "{{ route('create.sample_orders.update', ':id') }}".replace(':id', $('#sample_order_id').val()) : "{{ route('create.sample_orders.store') }}";
            var type = $('#sample_order_id').val() ? "PUT" : "POST";
            var data = $('#sample_order_id').val() ? $('#formUpdateSampleOrder').serialize() : $('#formAddNewSampleOrder').serialize();
            var id = $('#sample_order_id').val();
            $.ajax({
                url: url,
                type: type,
                data: data,
                ...
                success: function(response) {
                    if (response.status == "success") {
                        var redirectUrl = id ?
                            "{{ route('create.edit_product', ':id') }}".replace(':id', response.sample_order_id) :
                            "{{ route('create.index_product', ':id') }}".replace(':id', response.sample_order_id);
                        $('#formAddNewSampleOrder').trigger("reset");
                        sending(button, true);
                        showToast('success', response.msg);
                        // const url = "{{ route('create.edit_product', ':id') }}".replace(':id', response.sample_order_id);
        ...
        $('#saveSampleOrderProductBtn').click(function(e) {
            e.preventDefault();
            var button = $(this);
            sending(button);
            // Create a new FormData object with the form data
            var formData = new FormData($('#sampleOrderProductForm')[0]);
            // Collect product data
            $('#sampleOrderProductForm #sample_order_product tbody tr').each(function(index) {
                var row = $(this);
                formData.append(`sample_order_products[${index}][product_id]`, row.find('input[name="product_id[]"]').val());
            ...
            $('#sampleOrderProductForm #sample_order_service tbody tr').each(function(index) {
                var row = $(this);
                formData.append(`sample_order_services[${index}][service_id]`, row.find('select[name="service_id[]"]').val());
            ...
            var url = "{{ route('create.save_product') }}";
            $.ajax({
                url: url,
                type: "POST",
                headers: {
                ...
                success: function(response) {
                    if (response.status === "success") {
                        $('#sampleOrderProductForm').trigger("reset");
                        sending(button, true);
                        showToast('success', response.msg);
        ...
        $('#cancelBtn').click(function() {
            const opportunityId = $('#sample_order_id').val();
        ...
        $('input[name="delivery_type"]').on('change', function() {
            ...
            if (selectedType === 'delivery') {
                ...
                $('#delivery-fields').hide().find('input, select').val('');
                $('#delivery_country_id').val(null).trigger('change');
                $('#common-fields').show();
            } else {
                $('#delivery-fields, #common-fields').show();
            }
        });
    });
</script>