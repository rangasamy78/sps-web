<script type="text/javascript">
    $(function() {
        ...
        var table_accountfile = $('#saleOrderFile').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('sale_orders_files.list', ':id') }}".replace(':id', $id),
                type: 'GET',
                data: function(d) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#uploadsaleOrderFileForm')[0].reset();
                    } else {
        ...
        $('body').on('click', '.fileedit', function() {
            ...
            if (notesInput.prop('readonly') && fileTypeSelect.prop('disabled')) {
                ...
                notesInput.removeAttr('disabled').removeClass('bg-label-secondary');
        ...
        $(document).on('click', '.updateFile', function(e) {
            ...
            $.ajax({
                url: "{{ route('sale_orders_files.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response.status === 'success') {
        ...
        $('body').on('click', '.filedelete', function() {
            ...
            confirmDelete(id, function() {
                deletesaleOrderFile(id);
            });
        });

        function deletesaleOrderFile(id) {
            var url = "{{ route('sale_orders_files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
