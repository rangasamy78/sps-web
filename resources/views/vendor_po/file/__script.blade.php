...
$(function() {
    ...
    var table_vendorPofile = $('#vendorPoFile').DataTable({
        responsive: true,
        processing: true,
        serverSide: true,
        searching: false,
        ajax: {
            url: "{{ route('vendor_po_files.list', ':id') }}".replace(':id', $id),
            type: 'GET',
            data: function(d) {
    ...
    $('#saveFile').click(function(e) {
        ...
        $.ajax({
            url: '{{ route("vendor_po_files.store") }}',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.updateFile', function(e) {
        ...
        $.ajax({
            url: "{{ route('vendor_po_files.update', ':id') }}".replace(':id', id),
            type: 'POST',
            data: formData,
            success: function(response) {
                if (response.status === 'success') {
    ...
    $('body').on('click', '.filedelete', function() {
        ...
        confirmDelete(id, function() {
            deleteVendorPoFile(id);
        });

        function deleteVendorPoFile(id) {
            var url = "{{ route('vendor_po_files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
                ...
                success: function(response) {
                    if (response.status === "success") {
                        handleAjaxResponse(response, table_vendor_pofile);
                    } else {
