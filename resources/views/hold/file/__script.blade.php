<script type="text/javascript">
    $(function() {
        ...
        $id = $('#hold_id').val();
        var table_holdfile = $('#holdFile').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('hold.hold_files.list', ':id') }}".replace(':id', $id),
                type: 'GET',
                data: function(d) {
        ...
        $('#saveFile').click(function(e) {
            e.preventDefault();
            var form = $('#uploadHoldFileForm')[0];
            var formData = new FormData(form);
            $.ajax({
                url: '{{ route("hold.hold_files.store") }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        $(document).on('click', '.updateFile', function(e) {
            ...
            $.ajax({
                url: "{{ route('hold.hold_files.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response.status === 'success') {
        ...
        function deletevisitFile(id) {
            var url = "{{ route('hold.hold_files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
                ...
                success: function(response) {
                    if (response.status === "success") {
                        handleAjaxResponse(response, table_holdfile);
                    } else {
