<script type="text/javascript">
    $(function() {
        ...
        function calculateTotals() {
            ...
            $('#convertSampleOrderDatatable tbody tr').each(function() {
        ...
        $('#convertSampleOrderDatatable').on('change', '.tax, .quantity, .unit-price', function() {
            calculateTotals();
        });
        ...
        $('#saveCreateSampleOrder').click(function(e) {
            e.preventDefault();
            var button = $(this);
            sending(button);

            // Create FormData object with form data
            var formData = new FormData($('#formAddNewSampleOrder')[0]);

            // Collect product data
            $('#convertSampleOrderDatatable tbody tr.product-row').each(function(index) {
                var row = $(this);
                ...
                formData.append(`sample_order_products[${index}][is_tax]`, row.find('input[name="is_tax[]"]').prop('checked') ? 1 : 0);
            });

            // Collect visible service data
            $('#convertSampleOrderDatatable tbody tr.service-row').each(function(index) {
                var row = $(this);
                formData.append(`sample_order_services[${index}][service_id]`, row.find('input[name="service_id[]"]').val());
            ...
            // }
            // AJAX request
            var url = "{{ route('convert.save_convert_sample_order') }}";
            $.ajax({
                url: url,
                type: "POST",
                headers: {
                ...
                success: function(response) {
                    ...
                    if (response.status === "success") {
                        ...
                        window.location.href = "{{ route('create.sample_orders.show', ':id') }}".replace(':id', response.sample_order_id);
                    } else {
                        showToast('error', 'Failed to save the sample order.');
                    }
                },
