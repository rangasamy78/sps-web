<script type="text/javascript">
    $(function() {
        ...
        $id = $('#opportunity_id').val();
        var table_accountfile = $('#opportunityFile').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('opportunity_files.list', ':id') }}".replace(':id', $id),
                type: 'GET',
                data: function(d) {
        ...
        $('#upload').on('change', function() {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (fileInput) {
                    ...
                    let $select = $('#opportunityFileUploadRow');
                    let newRow = $(`
                <tr id="row-${rowCount}">
                    <td>
        ...
        $('#saveFile').click(function(e) {
            e.preventDefault();
            var form = $('#uploadopportunityFileForm')[0]; // Get the form element
            var formData = new FormData(form); // Create FormData object
            $.ajax({
                url: '{{ route("opportunity_files.store") }}', // Your upload route
                type: 'POST',
                data: formData,
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#uploadopportunityFileForm')[0].reset();
                    } else {
        ...
        $(document).on('click', '.updateFile', function(e) {
            ...
            $.ajax({
                url: "{{ route('opportunity_files.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response.status === 'success') {
        ...
        function deleteopportunityFile(id) {
            var url = "{{ route('opportunity_files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
