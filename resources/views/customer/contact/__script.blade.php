<script type="text/javascript">
    $(function() {
        var contactUrl =  "{{ route('customers.contacts.list', ':type_id') }}".replace(':type_id', $('#type_id').val());
        $.ajaxSetup({
            headers: {
        ...
        var table_contact = $('#customerContact').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ...
            columns: [{
                ...
                {
                    data: 'is_ship_to_address',
                    name: 'is_ship_to_address'
                },
                {
                    data: 'action',
                    name: 'action'
        ...
        $('#savecontactdata').click(function(e) {
            ...
            sending(button);
            $.ajax({
                url: "{{ route('customers.contacts.save') }}",
                type: "POST",
        ...
        $('#saveContactForm #is_ship_to_address').change(function() {
            if ($(this).is(':checked')) {
                $('.tax-code').show();
            } else {
                $('.tax-code').hide();
