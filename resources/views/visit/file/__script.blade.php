<script type="text/javascript">
    $(function() {
        ...
        $id = $('#visit_id').val();
        var table_accountfile = $('#visitFile').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('visit_files.list', ':id') }}".replace(':id', $id),
                type: 'GET',
                data: function(d) {
        ...
        $('#saveFile').click(function(e) {
            e.preventDefault();
            var form = $('#uploadvisitFileForm')[0]; // Get the form element
            var formData = new FormData(form); // Create FormData object
            $.ajax({
                url: '{{ route("visit_files.store") }}', // Your upload route
                type: 'POST',
                data: formData,
        ...
        $(document).on('click', '.updateFile', function(e) {
            ...
            $.ajax({
                url: "{{ route('visit_files.update', ':id') }}".replace(':id', id),
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response.status === 'success') {
        ...
        function deletevisitFile(id) {
            var url = "{{ route('visit_files.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
