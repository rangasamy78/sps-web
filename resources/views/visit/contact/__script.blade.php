<script type="text/javascript">
    $(function() {
        ...
        var table_visit_contact = $('#visitContact').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('visits.contacts.list', ':id') }}".replace(':id', $('#customer_id').val()),
                type: 'GET',
            },
            columns: [{
                    data: 'check',
            ...
            buttons: [{
                text: ' <span class="d-none d-sm-inline-block">Add to Visit</span>',
        ...
        $(document).on('click', '.addContactBtn', function() {
            ...
            $('.contact-checkbox:checked').each(function() {
                selectedContacts.push($(this).val()); // Push the value of the checked checkboxes (contact ids)
            });

            if (selectedContacts.length > 0 && visit_id) {
                $.ajax({
                    url: "{{ route('visits.contact.save') }}",
                    method: 'POST',
                    data: {
                        visit_id: visit_id,
                        contact_id: selectedContacts, // Send an array of selected contact IDs
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            response.contacts.forEach(function(contact) {
                                ...
                                $('#contact_' + contact.visit_contact_id).find('input[type="checkbox"]').prop('disabled', true);
                            });
                            $('.showContact').html(contactHTML);
                            table_visit_contact.ajax.reload(null, false); // Do not reset pagination
                        } else {
        ...
        function deleteVisitContact(id) {
            var url = "{{ route('visits.contacts.destroy', ':id') }}".replace(':id', id);
            $.ajax({
                url: url,
                type: "DELETE",
                data: {
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $(`#contact_${id}`)
                            ...
                            .prop('disabled', false);
                        $(`#contact_${id}`).remove();
                        table_visit_contact.ajax.reload(null, false);
                    } else {
                        showToast('danger', response.msg);
                    }
                },
