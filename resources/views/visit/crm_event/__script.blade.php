<script type="text/javascript">
    $(function() {
        ...
        var table_opportunity_event = $('#visitCrmEvent').DataTable({
            responsive: true,
            processing: true,
            serverSide: true,
            searching: false,
            ajax: {
                url: "{{ route('visit_events.list', ':id') }}".replace(':id', $('#visit_id').val()),
                type: 'GET',
                data: function(d) {
        ...
        $(document).on("change", ".event_check", function() {
            ...
            $.ajax({
                url: '{{ route("visit_events.update", ":id") }}'.replace(':id', id),
                type: 'PUT',
                data: data,
                dataType: 'json',
                success: function(response) {
