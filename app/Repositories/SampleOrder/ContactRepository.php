...
class ContactRepository
{
    ...
    {
        return SampleOrderContact::query()
            ->findOrFail($id);
    }
    public function save(array $data)
    {
        $visitId = $data['sample_order_id'];
        $contactIds = $data['contact_id'];
        foreach ($contactIds as $contactId) {
            if (!empty($contactId)) {
                SampleOrderContact::create([
                    'contact_id' => $contactId,
                    'sample_order_id' => $visitId,
                ]);
            }
        }
    }
    ...
    public function dataTable(Request $request, $id)
    {
        ...
        $arrData->map(function ($value) {
            ...
            $opportunityContact = SampleOrderContact::where('contact_id', $contactId)->first();
