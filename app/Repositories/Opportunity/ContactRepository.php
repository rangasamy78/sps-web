...
class ContactRepository
{
    ...
    public function dataTable(Request $request, $id)
    {
        ...
        $arrData->map(function ($value) {
            ...
            $opportunityContact = OpportunityContact::where('contact_id', $contactId)->first();
            if (!$opportunityContact) {
                ...
                $value->select_all = '<div class=""><button class="btn btn-danger btn-sm rounded-circle delete-contact p-2" data-id="' . $opportunityContact->id . '"><i class="fi fi-rr-cross"></i></button></div>';
            }
    ...
    public function dataTableBilltoContact(Request $request, $id)
    {
        ...
        $arrData->map(function ($value) {
            $contactId = $value->id ?? null;
            $opportunityContact = OpportunityContact::where('contact_id', $contactId)->first();
            $value->check = !$opportunityContact
                ? ''
            ...
            if ($opportunityContact) {
                $value->action .= "<a class='dropdown-item delete-contact text-warning' href='javascript:void(0);' data-id='$opportunityContact->id'><i class='fi fi-rr-cross-small'></i> Remove Contact from</a>";
            } else {
