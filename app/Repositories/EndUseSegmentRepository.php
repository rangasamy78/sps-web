...
class EndUseSegmentRepository implements CrudRepositoryInterface, DatatableRepositoryInterface
{
    ...
    public function getEndUseSegmentsList()
    {
    ...
    public function dataTable(Request $request)
    {
        ...
        $endUseSegment = $this->getEndUseSegmentsList();
        $total = $endUseSegment->count();

        $totalFilter = $this->getEndUseSegmentsList();
        if (!empty($searchValue)) {
            $totalFilter = $totalFilter->where('end_use_segment', 'like', '%' . $searchValue . '%');
        }
        $totalFilter = $totalFilter->count();
        $arrData = $this->getEndUseSegmentsList();
        $arrData = $arrData->skip($start)->take($rowPerPage);
        $arrData = $arrData->orderBy($columnName, $columnSortOrder);
        if (!empty($searchValue)) {
            $arrData = $arrData->where('end_use_segment', 'like', '%' . $searchValue . '%');
        }
        $arrData = $arrData->get();
