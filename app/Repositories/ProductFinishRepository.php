...
class ProductFinishRepository implements CrudRepositoryInterface, DatatableRepositoryInterface
{
    ...
    public function getProductFinishList()
    {
        $query = ProductFinish::query();
        return $query;
    }
    ...
    public function dataTable(Request $request)
    {
        ...
        $states = $this->getProductFinishList();
        $total  = $states->count();

        $totalFilter = $this->getProductFinishList();
        if (!empty($searchValue)) {
            $totalFilter = $totalFilter->where('product_finish_code', 'like', '%' . $searchValue . '%');
            $totalFilter = $totalFilter->orWhere('finish', 'like', '%' . $searchValue . '%');
        }
        $totalFilter = $totalFilter->count();

        $arrData = $this->getProductFinishList();
