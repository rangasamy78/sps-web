...
class SelectTypeSubCategoryRepository implements CrudRepositoryInterface, DatatableRepositoryInterface
{
    ...
    public function getSubCategoryNameList($id)
    {
        $SelectTypeSubCategorys = SelectTypeSubCategory::where('select_type_category_id', $id)
            ->take(4)
            ->pluck('select_type_sub_category_name')
            ->toArray();
