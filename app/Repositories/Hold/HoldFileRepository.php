...
class HoldFileRepository implements CrudRepositoryInterface
{
    use ImageUploadTrait;
    public function findOrFail(int $id)
    {
        return HoldFile::query()
            ->findOrFail($id);
    }

    public function store(array $data)
    {
        $accountId = $data['hold_id'];
    ...
    public function getHoldFileList($id)
    {
        $query = HoldFile::where('hold_id', $id);
        return $query;
    }
    ...
    public function dataTable(Request $request, $id)
    {
        ...
        $PriceListLabels = $this->getHoldFileList($id);
        $total           = $PriceListLabels->count();

        $totalFilter = $this->getHoldFileList($id);
        $totalFilter = $totalFilter->count();

        $arrData     = $this->getHoldFileList($id);
