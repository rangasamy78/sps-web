...
class ProductGroupRepository implements CrudRepositoryInterface, DatatableRepositoryInterface
{
    ...
    public function dataTable(Request $request)
    {
        ...
        $states = $this->getProductGroupList();
        $total = $states->count();

        $totalFilter = $this->getProductGroupList();
        if (!empty($searchValue)) {
            $totalFilter = $totalFilter->where('product_group_name', 'like', '%' . $searchValue . '%');
            $totalFilter = $totalFilter->orWhere('product_group_code', 'like', '%' . $searchValue . '%');
        }
        $totalFilter = $totalFilter->count();

        $arrData = $this->getProductGroupList();
        $arrData = $arrData->skip($start)->take($rowPerPage);
        $arrData = $arrData->orderBy($columnName, $columnSortOrder);

        if (!empty($searchValue)) {
            $arrData = $arrData->where('product_group_name', 'like', '%' . $searchValue . '%');
            $arrData = $arrData->orWhere('product_group_code', 'like', '%' . $searchValue . '%');
        }
        $arrData = $arrData->get();
