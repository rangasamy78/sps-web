...
class TaxCodeService
{
    ...
    public function getTaxCodeComponents($id)
    {
        $taxCodeComponents = DB::table('tax_components')
            ->join('tax_code_components', 'tax_components.id', '=', 'tax_code_components.tax_component_id')
            ->select('tax_code_components.id', 'tax_code_components.tax_code_id', 'tax_components.component_name', 'tax_code_components.rate')
            ->where('tax_code_components.tax_code_id', '=', $id)
            ->get();
        return $taxCodeComponents ?? '';
    }
